module.exports = {
  config: {
    ...
    aliases: [`pen`],
    version: "1.0",
    author: "Orochi Team",//cmd modified by Aryan Chauhan don't change my author name
    countDown: 0,
    role: 0,
    shortDescription: {
      vi: "",
...
onReply: async function({ api, event, Reply, getLang, commandName, prefix }) {
    ...
    if (isNaN(body) && body.indexOf("c") == 0 || body.indexOf("cancel") == 0) {
        ...
        for (const singleIndex of index) {
            if (isNaN(singleIndex) || singleIndex <= 0 || singleIndex > Reply.pending.length) return api.sendMessage(getLang("invaildNumber", singleIndex), threadID, messageID);
            api.sendMessage(`✅𝗖𝗢𝗡𝗡𝗘𝗖𝗧𝗘𝗗 𝗕𝗢𝗧\n\n🎀 Orochi Chatbot has been successfully connected 🫂🤍:\n\n💦 Join here to learn more about the bot: https://www.facebook.com/groups/2277594492449564/?ref=share\n\n📍 Type ${prefix}gc to enter the Messenger group 🙂🤍\n\n👉 Type ${prefix}commands to display the bot commands🫂🌝🤍\n\n🍒𝗧𝗘𝗔𝗠 𝗢𝗥𝗢𝗖𝗛𝗜-(𝗢𝗧)`, Reply.pending[singleIndex - 1].threadID);
            count+=1;
        }
