...
module.exports = {
  ...
  onStart: async function ({ api, event, args, message, usersData }) {
    ...
    if (!text) {
      return message.reply("⛔𝗜𝗠𝗔𝗚𝗜𝗡𝗘 𝗣𝗥𝗢𝗠𝗣𝗧𝗦\n\n❍➤ Please provide prompt.");
    }   
    ...
    message.reply("⚙️ 𝗚𝗘𝗡𝗥𝗘𝗔𝗧𝗜𝗡𝗚 𝗜𝗠𝗚\n\n✅| Creating your Imagination...", async (err, info) => {
  ...
  onReply: async function ({ api, event, Reply, usersData, args, message }) {
    ...
    try {
      if (reply >= 1 && reply <= 4) {
        ...
        message.reply("⛔ 𝗜𝗡𝗩𝗔𝗟𝗜𝗗 𝗥𝗘𝗣𝗟𝗬\\❍➤ Invalid image number. Please reply with a number between 1 and 4.");
        return;
      }
    } catch (error) {
      console.error(error);
